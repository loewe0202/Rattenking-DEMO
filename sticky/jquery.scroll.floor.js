(function(window){
	...
	function getItem(_list,newOptions){
		...
		_list.each(function() {
            ...
            item.$activeTop = $body.find(this).offset().top - $body.find(this).find('header').height() + 10;
            item.$scrollTop = $body.find(this).offset().top - $body.find(this).find('header').height() + 20;
            data.push(item);
        });
        return data;
	}
